(function () {
  var STYLE_ID = 'ww-reviews-carousel-styles-black';
  ...
  function ensureStyles() {
    ...
    style.id = STYLE_ID;
    style.textContent = [
      '.ww-carousel-widget{--ww-bg:#0b1220;--ww-surface:#111827;--ww-border:#243244;--ww-text:#e5e7eb;--ww-muted:#94a3b8;--ww-star:#fbbf24;--ww-primary:#38bdf8;--ww-shadow:0 16px 42px rgba(0,0,0,.45);--ww-gap:14px;--ww-cards-per-view:3;box-sizing:border-box;display:block;width:100%;max-width:100%;font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;border:1px solid var(--ww-border);border-radius:24px;padding:16px;background:linear-gradient(180deg,#0b1220 0%,#0f172a 100%);color:var(--ww-text);box-shadow:var(--ww-shadow);overflow:hidden}',
      ...
      '.ww-btn{width:36px;height:36px;border-radius:999px;border:1px solid var(--ww-border);background:#0b1220;color:#e2e8f0;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;font-size:17px;line-height:1;transition:all .2s ease}',
      '.ww-btn:hover{background:#1e293b;border-color:#334155}',
      '.ww-carousel-viewport{width:100%;overflow:hidden;border-radius:16px}',
      '.ww-carousel-track{display:flex;gap:var(--ww-gap);will-change:transform;transition:transform .45s ease}',
      '.ww-review-card{flex:0 0 calc((100% - (var(--ww-gap) * (var(--ww-cards-per-view) - 1))) / var(--ww-cards-per-view));min-height:230px;border:1px solid var(--ww-border);border-radius:16px;background:var(--ww-surface);padding:18px;display:flex;flex-direction:column}',
      ...
      '.ww-comment{font-size:14px;line-height:1.5;color:#cbd5e1;margin:0;display:-webkit-box;-webkit-line-clamp:5;-webkit-box-orient:vertical;overflow:hidden}',
      ...
      '.ww-dot{width:8px;height:8px;border-radius:999px;background:#334155;cursor:pointer;transition:all .2s ease}',
      '.ww-dot.is-active{width:22px;background:var(--ww-primary)}',
      '.ww-write-review{display:inline-flex;align-items:center;justify-content:center;padding:10px 14px;border-radius:10px;background:var(--ww-primary);color:#001018;text-decoration:none;font-size:13px;font-weight:700}',
      '.ww-empty{border:1px dashed var(--ww-border);border-radius:12px;padding:24px;text-align:center;color:var(--ww-muted);font-size:14px;background:#111827}',
      '.ww-error{border:1px solid #7f1d1d;background:#3b0a0a;color:#fecaca;border-radius:10px;padding:10px;font-size:13px}',
      '.ww-loading{font-size:13px;color:#93a3b8}',
      '@media (max-width:700px){.ww-carousel-widget{padding:14px;border-radius:18px}.ww-carousel-title{font-size:17px}.ww-btn{width:32px;height:32px}}',
    ].join('');
  ...
  function cardsPerView(viewport) {
    var width = Math.round((viewport.getBoundingClientRect && viewport.getBoundingClientRect().width) || viewport.clientWidth || 0);
  ...
  function setupLoopCarousel(track, dotsWrap, originalReviews, autoplayMs) {
    ...
    function build() {
      state.perView = Math.min(cardsPerView(track.parentNode), Math.max(1, originalReviews.length));
      track.style.setProperty('--ww-cards-per-view', String(state.perView));
  ...
  function renderWidget(target, payload, options) {
    ...
    if (typeof target.__wwCarouselCleanup === 'function') {
      target.__wwCarouselCleanup();
      target.__wwCarouselCleanup = null;
    }
    ...
    if (typeof ResizeObserver !== 'undefined') {
      resizeObserver = new ResizeObserver(rebuildCarousel);
      resizeObserver.observe(target);
      resizeObserver.observe(root);
      resizeObserver.observe(viewport);
    }

    target.__wwCarouselCleanup = function () {
      carousel.stopAutoplay();
      window.removeEventListener('resize', rebuildCarousel);
  ...
  function autoInit() {
    ...
    scripts.forEach(function (script) {
      if (script.__wwMountedCarouselBlack) return;
      if (script.src && script.src.indexOf('ReviewsCarouselBlack.js') === -1) return;
      script.__wwMountedCarouselBlack = true;
      mountFromScript(script);
    });
  }

  window.WidgetWizzardReviewsCarouselBlack = {
    init: autoInit,
  };
